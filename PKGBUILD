...
build() {
    local configure_args=(
        ...
        --with-bugurl=https://github.com/FlarebirdOS/gcc/issues
        ${configure_options}
    )
...
package_gcc() {
    ...
    ln -sfv /usr/libexec/gcc/${CHOST}/${pkgver%%+*}/liblto_plugin.so ${pkgdir}/usr/lib64/bfd-plugins/
    ...
    _pick gcclibs32bit ${pkgdir}/usr/lib64/gcc/${CHOST}/${pkgver%%+*}/32
}
...
package_gcc-libs() {
    ...
    for lib in libitm libgomp libquadmath; do
...
package_gcc-libs-32bit() {
    ...
    find ${pkgdir}/usr/include/c++/${pkgver%%+*} -mindepth 1 -path "${pkgdir}/usr/include/c++/${pkgver%%+*}/${CHOST}" -prune -o -exec rm -rf -- {} +
}
...
package_gcc-m2() {
    ...
    _pick m232bit ${pkgdir}/usr/lib64/gcc/${CHOST}/${pkgver%%+*}/32
